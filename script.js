...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InB3cWtwZXlranl1amhucmVsZWF4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODMyMzgxNDgsImV4cCI6MjA5ODgxNDE0OH0.6u2CKOPHcMtVeA2ph0QWTqgtvs-4BQJpsz6v2kCyOEY";
// =================================================================
...
const POSITION_CONFIG = {
  ...
  "Vice President D2": {
    shortLabel: "VP D2",
    hiddenFields: ["fc", "rfc", "const", "train"],
  },
  "Minister of Education D4": {
    shortLabel: "Edu D4",
    hiddenFields: ["fc", "rfc", "const", "res"],
  },
...
function getPositionConfig(positionName) {
  return (
    POSITION_CONFIG[positionName] || {
      shortLabel: positionName,
      hiddenFields: [],
    }
  );
}
...
// the single source of truth for isAdmin now â€” never set it directly.
function applyAuthSession(session) {
  isAdmin = !!session;
  currentStaffUsername = session
    ? staffEmailToUsername(session.user.email)
    : null;
  if (isAdmin) {
    updateAdminUI();
  } else {
...
function subscribeToRealtimeUpdates() {
  ...
  try {
    client
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "reservation_slots" },
        (payload) => {
          const affectedPosition =
            payload.new?.position || payload.old?.position;
    ...
    console.error(
      "Realtime subscription failed, relying on fallback polling:",
      err
    );
  }
}
...
function updateAdminUI() {
  ...
  if (adminBtn)
    adminBtn.innerText = currentStaffUsername
...
async function handleToggleReservation() {
  ...
  showCustomConfirm(
    `Are you sure to ${actionText} reservation for ${currentPosition}?`,
    async () => {
      const client = getSupabase();
      if (!client) return;
      ...
      setButtonBusy(
        toggleBtn,
        true,
        actionText === "open" ? "Opening..." : "Closing..."
      );
      ...
      try {
        ...
        showToast(
          `Reservation of ${currentPosition} now-${ newStatus ? "open" : "close" }!`,
          "success"
        );
      } catch (err) {
...
function handleEditFooter() {
  ...
  document.getElementById("edit-footer-name").value =
    currentName === "..." ? "" : currentName;
  document.getElementById("edit-footer-guild").value =
...
// auth.role() = 'authenticated' â€” this client-side isAdmin flag is only used
...
async function submitStaffLogin() {
  ...
  closeLoginModal();
  showToast(
    `Welcome back${ currentStaffUsername ? ", " + currentStaffUsername.toUpperCase() : "" }!`,
    "success"
  );
}
...
function detectAndSetTimezone() {
  ...
  const offsets = [
    -12, -11, -10, -9, -8, -7, -6, -5, -4, -3.5, -3, -2, -1, 0, 1, 2, 3, 3.5, 4,
    4.5, 5, 5.5, 5.75, 6, 6.5, 7, 8, 9, 9.5, 10, 10.5, 11, 11.5, 12, 12.75, 13,
    14,
  ];
  ...
  offsets.forEach((offset) => {
    ...
    const label = tzLabels[String(offset)]
      ? ` (${tzLabels[String(offset)]})`
      : "";
...
function renderTimeSlots() {
  ...
  for (let i = 0; i < 48; i++) {
    ...
    let appsInSlot = savedApplications.filter(
      (a) => String(a.time_slot).trim() === utcTimeStr
    );
    ...
    if (acceptedApp) {
      let detailBtn = `<span class="icon-tap-target" style="cursor:pointer; font-size: 1rem; vertical-align: middle;" title="View Details" onclick="openDetailsModal(${acceptedApp.id})">ðŸ”</span>`;
      let actionBtn = isAdmin
        ? `<div style="display:flex; align-items:center; justify-content:center; gap:6px;"> ${detailBtn} <button class="btn-apply btn-danger btn-compact" style="padding: 4px 8px; font-size: 0.75rem;" onclick="removeApp(${acceptedApp.id})">Remove</button> </div>`
        : detailBtn;

      row.innerHTML = ` <td>${actionBtn}</td> <td><strong>${utcTimeStr} UTC</strong><br><small style="color:#8a8d98;">Local: ${localTimeStr}</small></td> <td><span style="color:#22c55e; font-weight:bold;">Accepted</span></td> <td>${escapeHtml(acceptedApp.nickname)}</td> <td><span style="cursor:pointer; color:#3b82f6; text-decoration:underline;" onclick="copyToClipboard('${escapeHtml( acceptedApp.game_id )}')">${escapeHtml(acceptedApp.game_id)}</span></td> <td>${escapeHtml(acceptedApp.furnace_level) || "-"}</td> `;
    } else {
      ...
      row.innerHTML = ` <td>${actionBtn}</td> <td><strong>${utcTimeStr} UTC</strong><br><small style="color:#8a8d98;">Local: ${localTimeStr}</small></td> <td>${statusText}</td> <td>-</td> <td>-</td> <td>-</td> `;
    }
...
function buildStatDetailsHtml(app, compact = false) {
  if (compact) {
    return ` <div><span style="color:#8a8d98; margin-right: 10px;">Furnace Lvl:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.furnace_level) || "-" }</strong></div> <div><span style="color:#8a8d98; margin-right: 10px;">FC:</span> <strong style="color:#f59e0b;">${ escapeHtml(app.fire_crystal) || "0" }</strong></div> <div><span style="color:#8a8d98; margin-right: 10px;">RFC:</span> <strong style="color:#f59e0b;">${ escapeHtml(app.refined_fire_crystal) || "0" }</strong></div> <div><span style="color:#8a8d98; margin-right: 10px;">General:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.general_speedup) || "0" }</strong></div> <div><span style="color:#8a8d98; margin-right: 10px;">Const:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.construction_speedup) || "0" }</strong></div> <div><span style="color:#8a8d98; margin-right: 10px;">Research:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.research_speedup) || "0" }</strong></div> <div><span style="color:#8a8d98; margin-right: 10px;">Train:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.training_speedup) || "0" }</strong></div> `;
  }

  return ` <div><span style="color:#8a8d98;">Nickname:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.nickname) || "-" }</strong></div> <div><span style="color:#8a8d98;">Game ID:</span> <strong style="color:#3b82f6;">${ escapeHtml(app.game_id) || "-" }</strong></div> <div><span style="color:#8a8d98;">Furnace Level:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.furnace_level) || "-" }</strong></div> <hr style="border: 0; border-top: 1px solid #334155; margin: 4px 0;"> <div><span style="color:#8a8d98;">Fire Crystals (FC):</span> <strong style="color:#f59e0b;">${ escapeHtml(app.fire_crystal) || "0" }</strong></div> <div><span style="color:#8a8d98;">Refined Fire Crystals (RFC):</span> <strong style="color:#f59e0b;">${ escapeHtml(app.refined_fire_crystal) || "0" }</strong></div> <div><span style="color:#8a8d98;">General Speedup:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.general_speedup) || "0" } Days</strong></div> <div><span style="color:#8a8d98;">Construction Speedup:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.construction_speedup) || "0" } Days</strong></div> <div><span style="color:#8a8d98;">Research Speedup:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.research_speedup) || "0" } Days</strong></div> <div><span style="color:#8a8d98;">Training Speedup:</span> <strong style="color:#f1f5f9;">${ escapeHtml(app.training_speedup) || "0" } Days</strong></div> `;
}
...
function openWaitingModal(timeStr) {
  ...
  document.getElementById(
    "modal-title"
  ).innerText = `Waiting List - ${timeStr} UTC`;
  const modalTbody = document.getElementById("modal-table-body");
  modalTbody.innerHTML = "";

  const thead = modal.querySelector("thead tr");
  thead.innerHTML = ` <th style="padding: 5px 10px; text-align: left;">NICKNAME</th> <th style="padding: 5px 10px; text-align: left;">ID</th> `;

  let appsInSlot = savedApplications.filter(
    (a) => String(a.time_slot).trim() === timeStr && a.status === "Waiting"
  );

  appsInSlot.forEach((app) => {
    const mainRow = document.createElement("tr");
    let adminButtons = isAdmin
      ? ` <div style="margin-top: 4px;"> <button class="btn-apply btn-compact" style="background:#22c55e; font-size:0.7rem; margin-right:4px; animation: none;" onclick="acceptApp(${app.id})">Accept</button> <button class="btn-apply btn-danger btn-compact" style="font-size:0.7rem;" onclick="removeApp(${app.id})">Drop</button> </div> `
      : "";

    mainRow.innerHTML = ` <td style="padding: 5px 10px; text-align: left; font-weight: 500; white-space: nowrap;"> <span class="icon-tap-target" style="cursor:pointer; margin-right: 6px;" onclick="toggleDetails(${ app.id })">ðŸ”</span>${escapeHtml(app.nickname)} </td> <td style="padding: 5px 10px; text-align: left; white-space: nowrap;"> <span style="cursor:pointer; color:#3b82f6; text-decoration:underline;" onclick="copyToClipboard('${escapeHtml( app.game_id )}')">${escapeHtml(app.game_id)}</span> ${adminButtons} </td> `;
    modalTbody.appendChild(mainRow);

    const detailsRow = document.createElement("tr");
    detailsRow.id = `details-${app.id}`;
    detailsRow.style.display = "none";
    detailsRow.innerHTML = ` <td colspan="2" style="padding: 0; border: none;"> <div style="background: #151821; padding: 8px; margin: 2px 5px; border-radius: 4px; font-size: 0.8rem; text-align: left; border: 1px solid #334155;"> ${buildStatDetailsHtml(app, true)} </div> </td> `;
...
async function submitApplication() {
  ...
  const genSp =
    parseInt(document.getElementById("input-gensp").value.trim()) || 0;
  const constSp =
    parseInt(document.getElementById("input-constsp").value.trim()) || 0;
  const resSp =
    parseInt(document.getElementById("input-ressp").value.trim()) || 0;
  const trainSp =
    parseInt(document.getElementById("input-trainsp").value.trim()) || 0;

  if (!nickname) {
    showToast("Please enter In-Game Nickname!", "warning");
    return;
  }
  if (!gameId) {
    showToast("Please enter In-Game ID!", "warning");
    return;
  }
  if (!/^\d+$/.test(gameId)) {
  ...
  if (!furnaceLevel) {
    showToast("Please select Furnace Level!", "warning");
    return;
  }
...
async function acceptApp(id) {
  showCustomConfirm(
    "Accept this application? This will lock this time slot.",
    async () => {
      const client = getSupabase();
      if (!client) return;
      closeModal();
      try {
        ...
        showToast(
          "Failed to approve. It may have just been taken by someone else.",
          "error"
        );
        loadApplications();
      }
...
async function removeApp(id) {
  showCustomConfirm(
    "Delete this application record permanently?",
    async () => {
      const client = getSupabase();
      if (!client) return;
      closeModal();
...
function exportToCSV() {
  ...
  const headers = [
    ...
    "General SP (Days)",
    "Construction SP (Days)",
    "Research SP (Days)",
    "Training SP (Days)",
  ];
  const rows = savedApplications.map((app) => [
    sanitizeCsvField(app.position),
    sanitizeCsvField(app.time_slot),
    sanitizeCsvField(app.status),
    sanitizeCsvField(app.nickname || "-"),
    sanitizeCsvField(app.game_id || "-"),
    sanitizeCsvField(app.furnace_level || "-"),
    sanitizeCsvField(app.fire_crystal || "0"),
    sanitizeCsvField(app.refined_fire_crystal || "0"),
    sanitizeCsvField(app.general_speedup || "0"),
    sanitizeCsvField(app.construction_speedup || "0"),
    sanitizeCsvField(app.research_speedup || "0"),
    sanitizeCsvField(app.training_speedup || "0"),
  ]);
  ...
  link.setAttribute(
    "download",
    `SVS_Ministry_Export_${currentPosition.replace(/\s+/g, "_")}.csv`
  );
...
function startLiveClock() {
  ...
  setInterval(() => {
    ...
    const isScheduleVisible =
      schedulePage && !schedulePage.classList.contains("hidden");
...
async function loadRecentAccepts() {
  ...
  try {
    ...
    logListEl.innerHTML = "";
    data.forEach((item) => {
      const shortPos = item.position
        ? getPositionConfig(item.position).shortLabel
        : "Unknown";
      const logRow = document.createElement("div");
      logRow.className = "log-entry";
      logRow.innerHTML = ` <span>âœ… <span class="log-user">${escapeHtml( item.nickname )}</span> <span style="color: #8a8d98; font-size: 0.95em; margin-left: 5px;">${escapeHtml( item.time_slot )} UTC</span></span> <span class="log-pos">[${escapeHtml(shortPos)}]</span> `;
      logListEl.appendChild(logRow);
    });
  } catch (err) {
    console.error("Failed to load recent accepts:", err);
  }
}
...
function createSnowEffect() {
  ...
  if (snowContainer.querySelectorAll(".snowflake").length >= maxSnowflakes)
    return;
